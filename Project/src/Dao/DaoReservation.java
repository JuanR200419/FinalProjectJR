...
public class DaoReservation {
    ...
    public void insertReservation(Reservation reser) {
        ...
        if (very) {
            String insertSQL = "INSERT INTO reservations (id_reservation, id_user, id_rooom, id_stade_reservation, entry_date, exit_date, total_price) VALUES (?,?,?,?,?,?,?)";
...
//
    private boolean verifyDate(Reservation reser) {
        LocalDate entradaDate = LocalDate.parse(reser.getEntryDate());
        LocalDate salidaDate = LocalDate.parse(reser.getExitDate());
        if (!entradaDate.isBefore(salidaDate)) {
            System.out.println("entro al false de verify");
                    
            return false;
        }
         System.out.println("entro al true de verify");
