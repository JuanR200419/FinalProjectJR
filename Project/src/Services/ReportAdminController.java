...
public class ReportAdminController {
    ...
    public ArrayList<ReportAdmin> searchHotelWithReservations(int id) {
      ...
      return dao.searchRerverWithUser(id);
    ...
    public void createReporTreservationUser(ArrayList<ReportUser> listReportUser) throws FileNotFoundException, DocumentException, IOException {
        Document documento = new Document();
        String ruta = System.getProperty("user.home") + "/documents/";
        File directorio = new File(ruta);
        if (!directorio.exists()) {
        ...
        for (int i = 0; i < listReportUser.size(); i++) {
            addTableHeader(tabla, "Nombre de usuario ", String.valueOf(listReportUser.get(i).getNameUser()), boldFont, regularFont);
            addTableHeader(tabla, "Numero de habitacion", String.valueOf(listReportUser.get(i).getNumRoom()), boldFont, regularFont);
            addTableHeader(tabla, "Fecha de Entrada", String.valueOf(listReportUser.get(i).getEntry_date()), boldFont, regularFont);
            addTableHeader(tabla, "Fecha de Salida ", String.valueOf(listReportUser.get(i).getExit_date()), boldFont, regularFont);
            addTableHeader(tabla, "Precio Total", String.valueOf(listReportUser.get(i).getPriceTotal()), boldFont, regularFont);
            createBlankSpace(tabla); // Añadir espacio en blanco después de "Precio Total"
        }
